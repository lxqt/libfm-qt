...
void renameFile(FmFileInfo *file, QWidget *parent) {
  ...
  QByteArray nm = new_name.toLocal8Bit();
  GFile* dest = g_file_get_child(G_FILE(parent_gf), nm.constData());
...
_retry:
  ...
  QByteArray nm = new_name.toLocal8Bit();
  GFile* dest_gf = g_file_get_child(G_FILE(parent_gf), nm.constData());
  g_object_unref(parent_gf);
